...
export const PUT =async(req:NextApiRequest, res:NextApiResponseServerIo)=>{
    try {
        ...
        const {forumResponseId, forumId, forumsChannelId, serverId, sectionId } = req.query;
    ...
    forumResponse = await db.forumResponse.update({
        where: {
          id: forumResponseId as string,
        },
        data: {
    ...
    res?.socket?.server?.io?.emit(updateKey, forumResponse);
