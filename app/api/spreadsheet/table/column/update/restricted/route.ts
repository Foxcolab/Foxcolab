...
export const POST =async(req:NextRequest, res:NextResponse)=>{
    try {
        ...
        console.log(serverId, spreadsheetId, tableId, columnId, sectionId);
        ...
        const {restricted} = reqBody;
        console.log(restricted);

        if(!restricted) return NextResponse.json({error:"Column not found"}, {status:409});
